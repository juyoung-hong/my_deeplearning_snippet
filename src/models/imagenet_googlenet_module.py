...
class ImagenetLitModule(LightningModule):
    """Example of a `LightningModule` for Tiny ImageNet classification.
    ...
    def __init__(
        ...
        """Initialize a `ImagenetLitModule`.
    ...
    def model_step(
        ...
        """
        x, y = batch
        logits, aux1_logits, aux2_logits = self.forward(x)
        loss = (
            self.criterion(logits, y)
            + self.criterion(aux1_logits, y)
            + self.criterion(aux2_logits, y)
        )
